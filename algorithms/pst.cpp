...
//    freopen("1.in", "r", stdin);
    ...
    while (T--) {
        ...
        for (int i = 0; i < n; i++) {
            int x, y;
            cin >> x >> y;
            eggs.emplace_back(x, y);
        }

        sort(eggs.begin(), eggs.end());
        for (int i = 0; i < n; i++)
            xs.push_back(eggs[i].first);
        xs.erase(unique(xs.begin(), xs.end()), xs.end());

        for (int i = 0; i < n; i++) {
            int xi = lower_bound(xs.begin(), xs.end(), eggs[i].first) - xs.begin();
            pst.update(xi, eggs[i].second);
        }
        ...
        for (int i = 0; i < m; i++) {
            int l, r, b, t;
            cin >> l >> r >> b >> t;
            int xil = lower_bound(xs.begin(), xs.end(), l) - xs.begin();
            int xir = upper_bound(xs.begin(), xs.end(), r) - xs.begin() - 1;
            sum += pst.get_sum(xir, b, t) - pst.get_sum(xil - 1, b, t);
        }
        cout << sum << '\n';
    }
